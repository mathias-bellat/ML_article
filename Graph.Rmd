...
packages <- c("dplyr", "readr", "ggalluvial", "stringr", "plyr", "tibble", "googlesheets4", "rnaturalearth", "RColorBrewer")                                 ### Specify required packages

for (pkg in packages) { if (!(pkg %in% rownames(installed.packages()))) { install.packages(pkg) } }                    ### Install required packages
...
```
```{r info table, echo = FALSE}
library(knitr)
kable(info[1:5,], caption = "Five first row of infos table")
```
...
kable(obs[1:5,], caption = "Five first row of observation table")
```
...
```
```{r task table, echo = FALSE}
kable(task[1:5,], caption = "Five first row of task table")
```
...
kable(modelsList[1:5,], caption = "Five first row of the model table")
```

## 02 Basics statistics ##

```{r models year of publication}
# 02 Basics statistics ---------------------------------------------------------
...
```{r authors table, echo = FALSE}
kable(head(freq_df[order(-freq_df$Freq), ]), caption = "Six first row of msot prolific authors")
```

```{r categories per years}
...
```{r categories per years export, eval = FALSE}
# Export plot
...
```{r alluvial diagram, message=FALSE}
# 03 Alluvial diagram ----------------------------------------------------------
# 03.1 Models used #############################################################
